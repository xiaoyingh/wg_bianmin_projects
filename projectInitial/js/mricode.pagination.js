...
(function (factory) {
    ...
    var Page = function (element, options) {
        ...
        this.isLoadPage = this.options.remote.isLoadPage;
        this.loadDiv = this.options.remote.loadDiv;
        this.loadTotalName = this.options.remote.loadTotalName;
    ...
    Page.prototype = {
        ...
        remote: function () {
            ...
            if(this.isLoadPage){
            	pagination.remote.loadPage(this, this.currentUrl, this.currentParams, this.loadCallBack, this.options.remote.beforeSend, this.options.remote.complete,this.loadDiv);
            }else{
        ...
        loadCallBack: function (responseText) {
        	var total =$(this.loadTotalName).val();
            if (total == null || total == undefined)
                throw new Error("the response of totalName :  '" + this.options.remote.totalName + "'  not found.");
            total = utility.convertInt(total);
            this.total = total;
            ...
            if (this.currentPageIndex > 0 && lastPageNum - 1 < this.currentPageIndex) {
                ...
                if (typeof this.options.remote.success === 'function') this.options.remote.success(responseText);
                this.renderPagination();
            }
        },
    ...
    pagination.remote = {
        ...
        loadPage: function (pagination, url, data, success, beforeSend, complate,loadDiv) {
           $(loadDiv).load(url,data,function(responseText, textStatus, XMLHttpRequest){
        	   if(textStatus == 'success'){
        		   success.call(pagination,responseText);
        	   }
           });
        }
    };
