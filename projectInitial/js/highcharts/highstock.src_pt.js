...
dateFormat = function (format, timestamp, capitalize) {
	...
	var date = new Date(timestamp - timezoneOffset),
		...
		replacements = extend({
			...
			'p': hours < 12 ? '上午' : '下午', // Upper case AM or PM
			'P': hours < 12 ? '上午' : '下午', // Lower case AM or PM
...
defaultOptions = {
	...
	lang: {
		...
		decimalPoint: '.',
		numericSymbols: ['千', '百万', '十亿', '万亿', '千万亿', '百万万亿'], // SI prefixes used in axis labels
		resetZoom: '重置',
		resetZoomTitle: '重置缩放 1:1',
		thousandsSep: ','
	},
	global: {
		useUTC: true,
...
defaultOptions.lang = merge(defaultOptions.lang, {
	...
	rangeSelectorFrom: 'Hora de início',
	rangeSelectorTo: 'Hora de término'
});
...
RangeSelector.prototype = {
	...
	setInputValue: function (name, time) {
		...
		//console.log(options.inputEditDateFormat);
		//console.log(dateFormat(options.inputEditDateFormat || '%Y-%m-%d', this[name + 'Input'].HCTime));
		...
		//this[name + 'Input'].value = dateFormat(options.inputEditDateFormat || '%Y-%m-%d %H:%M:%S', this[name + 'Input'].HCTime);
		//this[name + 'DateBox'].attr({ text: dateFormat(options.inputDateFormat || '%Y年%m月%e日 %H时%M分%S秒', this[name + 'Input'].HCTime) });
	},
	...
	drawInput: function (name) {
		...
		input.onchange = function () {
			var inputValue = input.value,
				...
				//console.log(options.inputDateParser);
			...
			if (!isNaN(value)) {
				...
				//console.log(rangeSelector.minInput);
				//console.log(rangeSelector.maxInput);
				//console.log(rangeSelector.minInput.HCTime);
				//console.log(rangeSelector.maxInput.HCTime);
