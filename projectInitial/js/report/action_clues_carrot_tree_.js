//话题与行动线索分析
function action_clues_carrot_tree(d,id) {
        var data = d;//.msg;
        ...
        if (data &&data!=""&& data != "[]"&&data!="{}"&&data.length>0) {
            ...
            keywordTree("netizens_list_action_clues", foamtree, fin, id, action_clues_click_search);
        } else {
...
function action_clues_click_search(rid, keyword) {
  ...
  option.callback = function() {    	
      ...
      var uidUserName = $(".twitter-uid").attr("data-uid-username");
		if(uidUserName){
			if(uidUserName.lastIndexOf(",")>-1){
				uidUserName=uidUserName.substr(0,uidUserName.length-2)+"}";
			}
...
//  option.page_foot_html="";
	var obj = new iPage.ScrollLoadContainer("netizens_list_action_info_content", "sm_al/mod3/getSpeechSummaryForTopic", option).init();
}
