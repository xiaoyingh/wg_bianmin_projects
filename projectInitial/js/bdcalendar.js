...
(function() {
	...
	function V(Z) {
		...
		this.lunarMonthInChinese = this.lunarIsLeapMonth ? "闰" + F[Y.month - 1]
				: F[Y.month - 1];
		...
		this.showInLunar = (this.solarFestival == "") ? this.showInLunar
				: this.solarFestival;
		this.lunarFestival = U[this.lunarIsLeapMonth ? "00"
	...
	var X = (function() {
		...
		function g() {
			...
			debugger;
			Z.style.visibility = (k == i.solarYear && h == i.solarMonth) ? "visible"
					: "visible"
		}
		function a() {
		...
		function e(l, h) {
			for ( var k = 1901; k < new Date().getFullYear()+1; k++) {
				var j = S("OPTION");
				j.value = k;
				j.innerHTML = k;
				if (k == l) {
...
//			for ( var k = 1; k < new Date().getMonth()+2; k++) {
//				var j = S("OPTION");
//				j.value = k;
//				j.innerHTML = k;
//				if (k == h) {
...
//			}
		...
		function f(h) {
			...
			//		&& h.solarMonth === i.solarMonth && h.solarDate === i.solarDate) ? "hidden"
			//		: "visible"
		}
		return {
			init : function(h) {
	...
	var O = (function() {
		...
		function C(e) {
			...
			N("cm").innerHTML = "";
			for(var i=1;i<=a.lines;i++){
				var T= S("TR");
				var trID = "tr"+i.toString();
				T.className="tr";
				N("cm").appendChild(T);
				T.setAttribute("id",trID);    
				for(var k=1;k<=7;k++){
					var TD = S("TD");
					var tdID = "td"+((i-1)*7+k).toString();
					TD.className="td";
					N(trID).appendChild(TD);
			...
			for ( var b = 0; b < d.length; b++) {
				...
				var Y = S("DIV");
				//alert(Math.floor(b/7)+"and"+(b%7));
				Y.className = "cell";
				//Y.style.left = (b % 7) * 73.5 + "px";
				//Y.style.top = Math.floor(b / 7) * 45 + 2 + "px";
				var c = S("DIV");
				//c.className = "so";
				//c.style.color = ((b % 7) > 4 || d[b].isRest) ? "#c60b02"
				//		: "#313131";
				c.innerHTML = d[b].solarDate;
				Y.appendChild(c);
				c.setAttribute("name","day");
				var Z = S("DIV");
				//Z.style.color = "#666";
				...
				var tdID = "td"+(b+1).toString();
				N(tdID).appendChild(Y);
				//N("cm").appendChild(Y)
				if((b % 7) > 4 || d[b].isRest){
					c.className = "restday";
					Z.className = "restday-z";
				}else{
					c.className = "workingday";
					Z.className = "workingday-z";
					
				}
				if (e) {
					if (e.solarDate === d[b].solarDate) {
						//Y.style.cssText = "background:rgb(86, 170, 255);color: #fff;";
						Y.setAttribute("class","cell dangqian");
						c.setAttribute("style","color: #0400FF;");
						Z.setAttribute("style","color: #0400FF;");
					}
				} else {
					if (d[b].isToday) {
						//Y.style.cssText = "background:rgb(86, 170, 255);color: #fff;";
	...
	var E = (function() {
		...
		function G(c) {
			...
			var a = c.cell;
			var d = "#{solarYear}&nbsp;年&nbsp;#{solarMonth}&nbsp;月&nbsp;#{solarDate}&nbsp;日&nbsp;<br>#{solarWeekDayInChinese}";
			...
			C.innerHTML = Z(d, b);
			C.style.zIndex=100;
			var top = Math.floor(c.dateIndex/ 7) * 45 + 2 + a.offsetHeight - 5 + "px";
			if(c.dateIndex>3){
				top = Math.floor(c.dateIndex/ 7 -1) * 45 + 2 + a.offsetHeight + 3+ "px";
			}
			C.style.top = top;
			C.style.left =(c.dateIndex % 7) * 73.5  + a.offsetWidth - 5 + "px";
