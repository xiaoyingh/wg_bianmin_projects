...
//	this.flag = flag;
	...
	+'<button class="play" style="background:#FDFDFD;border:1px solid #C9C9C9;float:left;margin-top:-3px;">Play</button>'
	+'<div class="slider" style="margin-left: 140px;float:left; margin-right: 10px;margin-top: -24px;width:450px"></div></div>'
	+'<p style="color: #fff;position: absolute;top: 200px;z-index: 1; width:400px; text-align:center;margin-left:300px;padding-left:10px;">'
	+'<inpush type="text" class="amount" readonly style="border:0; color:#fff;font-family: fantasy;font-size:20px;color: #E4BFD8;">'
	+'</p>'; 
		this.legend = '<!-- 图例 --><div style="display:block"><div class="descGraph" style="left:50px;position: absolute;top:100px;">'+ 
			/*'<table>'+
			...
			+'<td class="name" style="color: #fff;font-size:14px;">认证</td></tr>'
			+'<tr style="display:none"><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[1]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td  class="name" style="color: #fff;font-size:14px;"></td></tr>'
			+'<tr style="display:none"><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[2]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;"></td></tr>'
			...
			+'<td class="name" style="color: #fff;font-size:14px;">非认证</td></tr>'	
			...
			+'<td class="name" style="color: #fff;font-size:14px;">未知</td></tr>'	
			...
			+'<tr><td lang=1><hr class="name-hr" onclick="pathShowOrNot(this)"  size=1 style="color: #fff;border-style:solid;width:50px;margin:6px 0px">  </td>'
			+'<td  class="name" style="color: #fff;font-size:14px;">转发关系</td></tr>'
			+'<tr><td lang=2><hr class="name-hr" onclick="ShowOrNot2(this)" size=1 style="color: #fff;border-style:dashed ;width:100%;margin:6px 0px"> </td>'
			+'<td  class="name" style="color: #fff;font-size:14px;">身份关联</td></tr>'
			+'</table></div></div>';*/
		'<table>'+
			'<tr><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0; border-style:solid;border-width:5px;border-color:'+this.color[0]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;">Autenticar</td></tr>'
			+'<tr style="display:none"><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[1]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td  class="name" style="color: #fff;font-size:14px;"></td></tr>'
			+'<tr style="display:none"><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[2]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;"></td></tr>'
			+'<tr><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[3]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;">Não autenticado</td></tr>'	
			+'<tr><td><div class="bd"><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[4]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;">Incógnito</td></tr>'	
			+'<tr style="display:none"><td><div class="bd" ><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[5]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;"></td></tr>'	
			+'<tr style="display:none"><td><div class="bd" ><div onclick="showOrNot(this)" style="width:4px;border-radius: 20px;height:0;border-style:solid;border-width:5px;border-color:'+this.color[6]+';opacity:1;overflow:hidden;" class="mtrack-color"/></div></td>'
			+'<td class="name" style="color: #fff;font-size:14px;"></td></tr>'	
			...
			+'<table style="border-color: #fff;">'
			+'<tr><td lang=1><hr class="name-hr" onclick="pathShowOrNot(this)"  size=1 style="color: #fff;border-style:solid;width:50px;margin:6px 0px">  </td>'
			+'<td  class="name" style="color: #fff;font-size:14px;">Encaminhar o relacionamento</td></tr>'
			+'<tr><td lang=2><hr class="name-hr" onclick="ShowOrNot2(this)" size=1 style="color: #fff;border-style:dashed ;width:100%;margin:6px 0px"> </td>'
			+'<td  class="name" style="color: #fff;font-size:14px;">Identidade está associada</td></tr>'
			+'</table></div></div>';
}
...
//		if($(n).parent().parent().next().html()=="认证"){
//			$(n).css("border-color",obj.color[0]);
//		}else if($(n).parent().parent().next().html()=="非认证"){
//			$(n).css("border-color",obj.color[1]);
//		}else if($(n).parent().parent().next().html()=="未知"){
//			$(n).css("border-color",obj.color[2]);
//		}
...
tranPath.prototype.changeBgColor = function(type){
	...
	if(type==1){//黑色
		...
		$("td.name").css("color","#fff");//修改图例的字体颜色
		$("hr.name-hr").css("border-top-color","#eee");//修改图例的字体颜色
		$("td.name").css("font-size","14px");//修改图例的字体颜色
	}else{//白色
		obj.path.style("stroke","rgb(15, 36, 189)");//修改线的颜色
		obj.text.select("text").transition().duration(10).style("fill", "#000");//修改字体颜色
		$("td.name").css("color","#000");//修改图例的字体颜色
		$("td.name").css("font-size","14px");//修改图例的字体颜色
		$("hr.name-hr").css("border-top-color","#000");//修改图例的字体颜色
	}
}
	...
	// fill = obj.scale//.category20();
	 ...
	 obj.svg.append('svg:rect') //平移框,提供放大缩小功能,拖动更轻松
        ...
        //节点---分成g,circle层级，就支持点击事件
     	...
     	obj.circle.enter().append("g").attr("class","nodes").on("mousedown", function (d){
        	//每次点击节点时  先将所有节点改为默认大小  在去改变当前点击的节点形状
        	$(".nodes").find("circle").attr("r","3"); 
        	$(".nodes").find("circle").css("stroke-width","1px");
        	$(".nodes").find("circle").css("stroke","rgb(142, 208, 232)");
...
//					       }else{//第一次点击
	...
	//					}
				    ...
				    //nodeArr.push(newA)
			        obj.links.forEach(function(link) {
			        	if(link.source==data && link.target!=data && $.inArray(link.target,nodeArr)<0){
			        		//判断是否已经包含了此target节点
			        		var targetCount = 0;
			        		if(nodeArr.length>0){
			        			for ( var k = 0; k < nodeArr.length; k++) {
			        				var targetObj = nodeArr[k];
			        				if(targetObj.nodeId == link.target.nodeId){
			        					targetCount++;
			        				}
								}	
			        		}
			        		if(targetCount==0){
				        		...
				        		n.fcount = link.fcount[0];
				        		n.sourceType = link.source_type;
				        		n.uuid = link.uuid;
				        		var newN = obj.getDealedData(n);
				        		nodeArr.push(newN)
			        		}
			        	}if(link.target==data && link.source!=data && $.inArray(link.source,nodeArr)<0){
			        		//判断是否已经包含了此source节点
			        		var targetCount = 0;
			        		if(nodeArr.length>0){
			        			for ( var k = 0; k < nodeArr.length; k++) {
			        				var targetObj = nodeArr[k];
			        				if(targetObj.nodeId == link.source.nodeId){
			        					targetCount++;
			        				}
								}	
			        		}
...
//						        }
     	        ...
     	        setTimeout(function () {
     	        	$(".nodes").eq(0).find("circle").attr("r","9"); 
        			$(".nodes").eq(0).find("circle").css("stroke-width","5px");
        			$(".nodes").eq(0).find("circle").css("stroke","pink");

//   	        	console.log(obj.circle.enter())
//   	        	console.log($('#netizen_network_tab').find('.nodes')[0])
//   	        	debugger
//					document.getElementsByClassName("nodes")[0].onclick();
//   	        	$('#netizen_network_tab').find('.nodes').eq(0).click();
     	        },100)
...
//				        	var tmp  =style_str +"</span>:<img src='images/close.png'  class='close_my_imgs'  style='cursor:pointer;margin-left: 300px;'></div><div>站点："+newD.site+"&nbsp;&nbsp;&nbsp;粉丝数:"+newD.fans+"&nbsp;&nbsp;&nbsp;认证类型:"+newD.type;
				        	...
				        	div.style('display', 'none'); //解决tooltip出现白色横线的bug
...
tranPath.prototype.getDealedData = function(d){
	 ...
	 if(sfen.indexOf("5")>-1)gv2+="其他,";
	 if(sfen.indexOf("6")>-1)gv2+="媒体机构,";
	 if(gv.indexOf(",")>-1)
